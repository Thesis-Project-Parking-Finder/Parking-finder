...
export default function ExtendParkingTimer({ route }) {
  ...
  const extendHours = () => {
    if (userObject.currencyPoints >= value * 100) {
      const spotRef = doc(db, "bookings", `${route.params.extend.id}`);
      updateDoc(spotRef, {
        Duration: route.params.extend.Duration + value,
      });
      const spotRef1 = doc(db, "users", `${auth.currentUser.uid}`);
      updateDoc(spotRef1, {
        currencyPoints: userObject.currencyPoints - value * 100,
      });
      navigation.navigate("Map");
    } else {
      navigation.navigate("CantBookings");
    }
  };
  return (
    <SafeAreaView style={styles.Wrapper}>
      ...
      <View style={styles.card1}>
        ...
        {checkedCustom2 && !checkedCustom ? (
          <Image
            style={styles.imageL}
            source={{ uri: "https://img.icons8.com/cotton/344/ruble--v2.png" }}
          />
        ) : (
          <Image
            style={styles.imageL}
            source={{ uri: "https://img.icons8.com/dusk/452/banknotes.png" }}
          />
        )}
      ...
      <TouchableOpacity style={styles.Frame178} onPress={extendHours}>
